...
function auditTCNACompliance() {
  ...
  const keyPages = ['index.html', 'pages/services.html', 'pages/build/index.md'];
  let tcnaFound = 0;
...
function auditNJHICCompliance() {
  ...
  try {
    const files = ['index.html', 'pages/services.html', 'pages/about.html', '_includes/footer.html'];
...
function auditBuildPhaseCompliance() {
  ...
  try {
    ...
    files.forEach(file => {
      ...
      const hasInternalLinks = bodyContent.includes('/pages/build/') || bodyContent.includes('](/');
      if (!hasInternalLinks) {
        findings.push({
          level: 'info',
          file: 'pages/build/' + file,
...
function runAudit() {
  ...
  let markdown = '# Tillerstead Compliance Audit Report\n\n'
    + '**Generated:** ' + new Date().toISOString() + '\n'
    + '**License:** NJ HIC #' + REPORT.complianceLicense + '\n'
    + '**Site:** ' + REPORT.siteUrl + '\n\n'
    + '## Summary\n\n'
    + '| Category | Status |\n'
    + '|----------|--------|\n'
    + '| TCNA 2024 | ' + REPORT.results.tcna.status.toUpperCase() + ' |\n'
    + '| NJ HIC | ' + REPORT.results.njHic.status.toUpperCase() + ' |\n'
    + '| WCAG 2.1 | ' + REPORT.results.wcag.status.toUpperCase() + ' |\n'
    + '| Build Phase | ' + REPORT.results.buildPhase.status.toUpperCase() + ' |\n'
    + '| Metadata | ' + REPORT.results.metadata.status.toUpperCase() + ' |\n'
    + '| Color Contrast | ' + REPORT.results.colors.status.toUpperCase() + ' |\n\n'
  ...
  writeFileSync(
    join(ROOT, 'compliance-audit-report.md'),
    markdown
  );
